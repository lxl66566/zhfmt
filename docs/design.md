...
- 前向判定 `peek_forward_class`：跳过 `Soft`，遇 `Space`/`Hard`/EOF 返回无；
  多字节 run 的右边界直接检查下一字节是否 ASCII 字母数字（零解码）。
- 后向判定 `lookback_class`：跳过 `Soft` 与附着于前词的半角尾随标点
  （`,` `.` `;` `:` `?` `%` `+`，因此 `中文,english` → `中文, english`、
  `C++语言` → `C++ 语言`）；`Hard` 或区域起点意味着该边界已被先前事件处理，保留传入 `prev`。
...
2. 只分类 run 的**首字符**（且仅当 `prev == Latin` 时才需要，否则跳过这次解码）
   与**尾字符**（可能参与右侧边界）。
3. 右侧边界直接检查下一字节是否为 ASCII 字母数字（零解码）。`*`/`~` 不再被
   透视：它们是 wake byte，其两侧边界由强调处理器统一决定——否则空格会落入
   marker 与内容之间，破坏 CommonMark flanking 使强调失效。
...
| fenced code block | 作为巨型 code span 由反引号 run 配对跳过 | 同上 |
| 缩进代码块 | 空行/文首之后的 4 空格或 tab 缩进行起，连续的缩进行与空行整体跳过 | 块级构造，两侧由换行阻断 |
| YAML front matter | 文首 `---` 行至 `---`/`...` 行，整体原样跳过（hardcode 字符串） | 同上 |
| 链接 `[t](url)` | text 按正文，url 整体跳过（容忍一层嵌套括号） | 由 text 决定，url 不影响 |
| 裸 `[...]`（引用式链接/脚注定义/字面方括号） | 无法本地判定，不透明原子（与括号、引号一致） | 两侧一律阻断 |
| HTML `<…>` / `<!--…-->` / `<?…>` / autolink | 不透明原子，含属性值（`title="中文"`、`<template #槽位>`）永不扫描 | 两侧一律阻断 |
| 脚注 `[^id]` | 不透明原子（`]` 后跟随 `(` 时按普通链接处理） | 两侧一律阻断 |
| 强调 `*…*`/`**…**`/`~~…~~` | 成对（含 crude CommonMark 左右 flank 检查，拒绝列表符）时作为包装：内部递归格式化。**配对搜索原子地跳过 code span 与 HTML 标签/注释**——标记内部的 `*`/`~` 是代码或标记语言，绝不参与配对（否则配对跳转会落入 span 内部，使后续反引号配对整体错位，把空格插进 code span）。closer 超出配对窗口时按 pending opener 乐观配对（尾部 memchr + memo 保持线性） | 空格只加在标记**外侧**，由内部内容决定（内部含 code span 时由 span 内部首/尾字符决定） |
| 未配对 `*`/`~` | 字面标记 | 空格贴 CJK 侧；任一侧已有空白则不动（保证幂等） |
...
2. 右边界零解码前窥（混合文本高事件密度场景的主要收益）。
3. SIMD 续扫 + 内联 SWAR 混合：干净 ASCII 区段与长中文 run 的扫描带宽。
4. 单次 open + 惰性分配：端到端 IO 与分配开销。

## 9. 已知限制（v1）

- fenced code block 由反引号 run 配对识别：围栏内部若含同长 ``` run
  （例如嵌套示例），配对会提前结束，剩余内容按正文格式化。
- 图片 `![alt](url)` 的 `!` 阻断左边界。
- 缩进代码块的判定是启发式（空行/文首 + 4 空格或 tab 缩进），不跟踪列表
  嵌套层级：误报方向是把列表续段当作代码跳过（保守，不误改），漏报方向
  是标题后直接跟随的缩进代码仍会被格式化。
- 强调配对是"crudely CommonMark"：只做左右 flank 的空白检查，
  不做完整的 delimiter stack 算法；`*` 超过 3 个的 run 不参与超窗乐观配对。
