...
[Prettier](https://prettier.io) 3 插件：在中文与英文/数字之间自动添加空格（盘古之白），补上 Prettier 3 移除该功能后的空缺。格式化引擎是 [zhfmt](https://github.com/lxl66566/zhfmt) 编译的 WebAssembly，与 CLI 行为完全一致。
...
{
  "plugins": ["prettier-plugin-zhfmt"],
  "overrides": [{ "files": ["*.md"], "options": { "parser": "zhfmt" } }]
}
```
