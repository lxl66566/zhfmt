...
5. 结构处理：
   - backtick code span（含成对的 fenced code block）内部原样跳过，其左右边界分别由内部首/尾内容字符决定；缩进代码块（空行/文首后 4 空格或 tab 缩进）与文首 YAML front matter 整体跳过——内容是 hardcode，不是正文；`[text](url)` 的 text 按正文处理，url 整体跳过且不影响边界
   - HTML 标签/注释（`<...>`、`<!-- ... -->`）、脚注引用 `[^id]` 与裸 `[...]` 方括号（引用式链接/字面括号注解）是不透明原子：内部（含 `title="中文"`、`<template #槽位名>` 等）永不扫描，两侧边界一律阻断
   - 强调符号（`*...*`、`**...**`、`~~...~~`）成对时作为整体：边界由内部内容决定，空格只加在标记**外侧**（`CG**鉴赏**` -> `CG **鉴赏**`），内部仍正常格式化；未配对的标记按字面文本处理，空格贴 CJK 侧
   - 附着于前词的半角尾随标点（`,` `.` `;` `:` `?` `%` `+`）向后透视，空格加在标点之后（`中文,english` -> `中文, english`、`C++语言` -> `C++ 语言`）
